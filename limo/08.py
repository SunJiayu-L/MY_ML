...
#数据集，读取数据集features的个数,然后将0-999随机打乱，每次取一批（batch_size个）数据出来，
#通过yield返回一对features和labels，因为有yield，所以每次生成的都被保存起来了。
...
#之前定义损失函数时没有求均值，此处求均值
#使用sgd只能求的局部最小值，但线性规划中损失函数loss只有一个最小值，故可以使用这个函数
